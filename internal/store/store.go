...
func Migrate(ctx context.Context, dsn string) error {
	...
	if _, err := db.ExecContext(ctx, "SELECT pg_advisory_lock(8241150)"); err != nil {
		return fmt.Errorf("store: migrate lock: %w", err)
	}
	defer func() { _, _ = db.ExecContext(ctx, "SELECT pg_advisory_unlock(8241150)") }()
...
// transactions (recursor reconciler, challenge sweeper, audit writing) set app.system so the RLS
