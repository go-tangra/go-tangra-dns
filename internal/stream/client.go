// Package stream delivers live DNS events to signed-in people: events are
...
// from their Last-Event-ID. Payloads are ids and metadata only (never record
// values, API keys or configuration; see internal/events).
package stream
...
func RateKey(kind, subject string, at time.Time) string {
	return "dns:rl:" + kind + ":" + subject + ":" + strconv.FormatInt(at.Unix()/60, 10)
}
