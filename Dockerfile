...
# DNS service image: builds the Vue remote, embeds it (-tags ui), and
# produces a slim runtime carrying dnssvc. Build context is the repo root
...
COPY services/dns/ui/ ./services/dns/ui/
RUN npm run -w services/dns/ui build

FROM golang:1.26-alpine AS build
RUN apk add --no-cache git ca-certificates
WORKDIR /src
COPY . .
COPY --from=ui /w/services/dns/ui/dist ./services/dns/ui/dist
WORKDIR /src/services/dns
ENV CGO_ENABLED=0 GOFLAGS=-buildvcs=false
RUN go build -tags "ui" -o /out/dnssvc ./cmd/dnssvc

FROM alpine:3.20
RUN apk add --no-cache ca-certificates postgresql-client && adduser -D -u 10001 app
COPY --from=build /out/dnssvc /usr/local/bin/
COPY services/dns/deploy /app/deploy
WORKDIR /app
USER app
ENTRYPOINT ["dnssvc"]
