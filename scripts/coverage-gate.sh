#!/usr/bin/env bash
# Service coverage gate: >=80% overall, 100% for the packages that decide
# access, hold key material or turn untrusted input into names sent to
# PowerDNS (dns: the authorizer, sealed envelopes and the validate package —
...
for p in "${SECURITY_PKGS[@]}"; do
  ...
  echo "coverage: $p ${pct}%"
  if [[ "$pct" == "n/a" ]]; then echo "coverage: $p has no coverage data" >&2; fail=1
